...
db.dl_py_pa  = convert(db.dl_py,'a'); % "log change GDP deflator AK 7/1
...
db.def_a = convert(db.def_y,'a'); % unweighted by Q GDP, but series s.a.!
...
db.gdem_y_a = convert(gdem_y,'a', 'method', 'last');
...
db.gdem_y_nom_a = convert(gdem_y_nom,'a', 'method', 'last')*100; % annual share GDP w deflators fr AUX .model
cons_y_nom = (db.ncons+db.ncons{-1}+db.ncons{-2}+db.ncons{-3})/...
    (db.ny+db.ny{-1}+db.ny{-2}+db.ny{-3});
db.cons_y_nom_a = convert(cons_y_nom,'a', 'method', 'last')*100; % annual share GDP w deflators fr AUX .model
inv_y_nom = (db.ninv+db.ninv{-1}+db.ninv{-2}+db.ninv{-3})/...
    (db.ny+db.ny{-1}+db.ny{-2}+db.ny{-3});
db.inv_y_nom_a = convert(inv_y_nom,'a', 'method', 'last')*100; % annual share GDP w deflators fr AUX .model
exp_y_nom = (db.nexp+db.nexp{-1}+db.nexp{-2}+db.nexp{-3})/...
    (db.ny+db.ny{-1}+db.ny{-2}+db.ny{-3});
db.exp_y_nom_a = convert(exp_y_nom,'a', 'method', 'last')*100; % annual share GDP w deflators fr AUX .model
imp_y_nom = (db.nimp+db.nimp{-1}+db.nimp{-2}+db.nimp{-3})/...
    (db.ny+db.ny{-1}+db.ny{-2}+db.ny{-3});
db.imp_y_nom_a = convert(imp_y_nom,'a', 'method', 'last')*100; % annual share GDP w deflators fr AUX .model

db.tb_rat_a = convert(db.tb_rat,'a'); % resource balance in % of GDP AK 7/1

db.dl_md_a  = convert(db.dl_md,'a'); % average 4 Q of annualized Q-on-Q growth

% debt ratios
% db.debt_y = (db.debt_fcy_y + db.debt_lcy_y)/4;
db.debt_y_a = convert(db.debt_y,'a', 'method', 'last');
db.debt_fcy_a = convert(db.debt_fcy_y/4,'a', 'method', 'last');
db.debt_lcy_a = convert(db.debt_lcy_y/4,'a', 'method', 'last');
