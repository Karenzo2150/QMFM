...
opts.addDataFile    = "2024-09-01"; % has CBO proj RIR-US and old GTM numbers
opts.compRound      = "main/2023-11-Nov"; % comparison round
%opts.compRound      = "develop/2024-09-Sept-ZR"; % comparison round
...
opts.readModel.run                    = 0; % or true = 1; false = 0

opts.readData.run                     = 0;

opts.filterHistory.run                = 0;

opts.reportModel.run                  = 0;
...
opts.reportHistory.shocks.run         = 0;
...
opts.reportModel.varDecomp.histRange = qq(2006, 1) : qq(2024, 2);
...
% Range of comparison
opts.reportModel.stdComp.range = qq(2006, 1) : qq(2024, 2);


%% Historical filter options

% Filtered range
opts.filterHistory.range = qq(2006, 1) : qq(2024, 2); % default 2024Q2 if data; extend filter for shocktuning +2Q
...
% Table range
opts.filterHistory.rangeTable = qq(2019, 4) : qq(2024, 2); % default 2024Q2; extend filter for shocktuning, +2Q
