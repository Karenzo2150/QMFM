...
opts.roundId        = "2024 September forecast round";
opts.modelFile      = "minecofin";
opts.mainDataFile   = "2024-09-18"; % name of data/Rwametric file 
opts.addDataFile    = "2024-09-01"; % addtional dataset for the model containing exo. vars incl. foreign IR i.e., US Real IR.
opts.compRound      = "main/2023-11-Nov"; % comparison round
...
opts.readModel.run                    = 1; % or true = 1; false = 0
...
opts.reportModel.stdComp.run          = 0;
opts.reportModel.equations.run        = 1;

opts.reportHistory.run                = 1;
opts.reportHistory.obs.run            = 0;
opts.reportHistory.trends.run         = 1;
opts.reportHistory.eqDecomps.run      = 1;
opts.reportHistory.shockDecomps.run   = 0;
opts.reportHistory.shocks.run         = 1;

opts.calcHistForecast.run             = 1;

opts.reportHistForecast.run           = 1;

opts.calcForecast.run                 = 1;

opts.reportForecast.run               = 1;

opts.reportCompareData.run            = 1;

opts.reportCompareForecast.run        = 1;

opts.reportForecastChangeDecomp.run   = 1;
...
opts.reportModel.varDecomp.histRange = qq(2006, 1) : qq(2024, 1);
...
% Range of comparison
opts.reportModel.stdComp.range = qq(2006, 1) : qq(2024, 1);


%% Historical filter options

% Filtered range
opts.filterHistory.range = qq(2006, 1) : qq(2024, 2); % default 2024Q1 if data; extend filter for shocktuning +2Q
...
% Table range
opts.filterHistory.rangeTable = qq(2019, 3) : qq(2024, 2); % default 2024Q1; extend filter for shocktuning, +2Q

% Plot range
opts.filterHistory.rangePlot = qq(2015, 1) : qq(2024, 2); % same
...
opts.histForecast.range = qq(2016, 1) : qq(2023, 2);
...
% Horizon of forecast
opts.forecast.range = qq(2024, 3) : qq(2028, 4); % extended forward each Q if new data
...
opts.forecastReport.plotRange = qq(2017, 1) : qq(2028, 4); % extended 2028Q4

% Range of the highlighted area i.e. highlight recent past
opts.forecastReport.highlightRange = qq(2015, 1) : qq(2024, 2); %  

% Range of the tables in the forecast report
opts.forecastReport.tableRange = qq(2023, 4) : qq(2027, 4); % extend 2028Q4
