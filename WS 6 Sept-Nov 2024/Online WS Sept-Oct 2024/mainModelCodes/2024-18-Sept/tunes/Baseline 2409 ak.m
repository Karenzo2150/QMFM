...
pln = exogenize(pln,  rngExog, 'd4l_y');
pln = endogenize(pln, rngExog, 'shock_l_cons_gap');
tunes.d4l_y(rngExog) = 100 * log(1 + [8]/100); % tune with nowcast of Aug 2024 for Q2-3
...
tunes.shock_l_inv_gap(rngExog) = [ 0.6,  -0.3]; % need to be read from ext.filter results
tunes.shock_l_exp_gap(rngExog) = [ 1.3,  -0.6]; % 
tunes.shock_l_imp_gap(rngExog) = [-0.03, -0.0]; %
