...
% we tuned INV gap (NISR INV data are ignored 2023Q3-2024Q2) as +shock huge (+60%)
rngTune = qq(2024, 1);
dbObs.tune_l_inv_gap(rngTune) = 25;
% we tuned EXP gap (NISR EXP data are ignored 2023Q3-2024Q2)as -shock huge (-35%)
rngTune = qq(2024, 2);
dbObs.tune_l_exp_gap(rngTune) = -15;
...
dbObs.tune_d4l_y (rngTune) = 100 * log(1 + [8]/100); % Nowcast Aug'24

%% CPI option to hard- or soft-tune CPIs based on NBR estimate or judgement and expected shocks
% CPI_core can be hard-tuned to match any NBR CPI forecast for 2024Q3--not used now
%rngTune = qq(2024,3) : qq(2024,3);
...
% rngTune = qq(2024, 3);
% dbObs.tune_dl_cpi_food(rngTune) = 100*log(1 + 0/100);
...
% rngTune= qq(2024, 3);
% dbObs.tune_dl_cpi_ener(rngTune) = 100*log(1 + 0/100);
...
% rngTune = qq(2024, 3);
% dbObs.tune_dl_cpi(rngTune) = 100*log(1 + 0/100);
...
rngTune = qq(2024,4);
dbObs.tune_dl_s(rngTune) = 100 * log(1 + 0.09); % tune dl_s, add in model tune_ declaration
