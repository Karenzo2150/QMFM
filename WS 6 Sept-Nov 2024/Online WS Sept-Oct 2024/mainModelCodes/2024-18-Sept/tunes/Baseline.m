...
% further shocks to avoid too low prices thru end-2026 (as we did in Dec 2023)
rngExog = qq(2024, 4) : qq(2026, 4);
tunes.shock_dl_cpi_core = Series(rngExog, [0.8, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4]); 

% 3b. CPI_food soft-tune 2024Q4 a.f. if further shock expected  
%rngExog = qq(2024, 4): qq(2024,4);
%tunes.shock_dl_cpi_food = Series(qq(2024,4):qq(2025,2), [1, 1, 1]); 
...
rngExog = qq(2024, 3) : qq(2024, 4);
tunes.shock_l_inv_gap(rngExog) = [ -0.2937,  -1.2459]; % need to be read from ext.filter results
tunes.shock_l_exp_gap(rngExog) = [ -0.64863, -2.7622]; % 
tunes.shock_l_imp_gap(rngExog) = [-0.059848,  0.020335]; %
...
tunes.dl_s(rngExog) = 100 * log(1 + 0.09); % equivalent! tune dl_s and also added tune_dl_s to minecofin.odel
