...
varNames = [
  ...
  "ngcons_Bdgt" %EvM added to obs and store nominal budget govt cons for comparison with NA govt cons
  "nimp_rwf_goods" 
  "nexp_rwf_goods"
  ];
...
%Ev added to adjust budget govCons as 25% of budget govInv
db.ngcons_Bdgt (rngAdj1) = db.ngcons_Bdgt(rngAdj1) + (0.25 * db.nginv(rngAdj1));
...
% EvM added to store bugdet gcons in db 
db.gcons_Bdgt    = db.ngcons_Bdgt  / pgcons;      % real Budget govcons using NA gov deflator       
db.ngdem_Bdgt    = db.ngcons_Bdgt  + db.nginv;    %EvM added to compute the nominal gdem using gcons from budget 
db.gdem_Bdgt     = db.gcons_Bdgt   + db.ginv;     %real govcons using real budget govcons 

% db.ndef_lcy = db.ndebt_lcy - db.ndebt_lcy{-1}; % def-fin can't be calc fr chDebt
% db.ndef_fcy = db.ndebt_fcy  - db.ndebt_fcy{-1}; read fr Fiscal, then ratios
...
%e add now Eurobond to net govt borr (though we did not do that before) but we are not using YET net borr govt from BOP, and only use that from Budget (large Q discrepancies between the two!)
%net private borr (excl. Eurobond, incl. IMF-BNR), Eurobond, net govt borr (incl. IMF budget (&SDRall), excl. Eurobond)
...
db.dBG_bop_usd = db.dBG_bop_usd + db.dBPG_usd_eurobond; % add in Eurobonds (separate line in BOP) to net govt borrBOP comparable to FISCal
...
% EvM added to calc govt cons ratios in nominal terms 
db.gcons_Bdgt_y  = 100 * db.ngcons_Bdgt / db.ny;
db.gdem_Bdgt_y  = 100  * db.ngdem_Bdgt  / db.ny;
...
% !!! Temporary fix !!!
db.inv(qq(2024,  3) : qq(2024, 2))    =  NaN;
...
varNames = [
  ...
  %EvM added for sa of govt dem and gcons from bugdet 
  "gcons_Bdgt"
  "ngcons_Bdgt"
  "gcons_Bdgt_y"
  "gdem_Bdgt"
...
dbObs.obs_rstar_tnd    = hpf(dbObs.obs_rstar_US_cbo); % this is new rstar_tnd from CBO (in addData...)
...
%EvM added for storing log trans of sa of budget govt dem and cons 
dbObs.obs_l_gdem_Bdgt     = db.l_gdem_Bdgt_sa;       %logged and sa real govdem using real  govcons and govinv
dbObs.obs_l_gcons_Bdgt     = db.l_gcons_Bdgt_sa;     %logged and sa real govcons
...
dbAux.ngcons_Bdgt   = db.ngcons_Bdgt; %EvM added to store govt demand calc used govt cons from budget
...
%EvM added to store  govt cons and demand 
dbAux.gcons_Bdgt_y       = db.gcons_Bdgt_y_sa;
dbAux.gcons_Bdgt         = db.gcons_Bdgt_sa;
dbAux.gdem_Bdgt_y        = db.gdem_Bdgt_y_sa;
dbAux.gdem_Bdgt          = db.gdem_Bdgt_sa;
...
varNames = [
  ...
  "s"
  "z"
  %"gdem_Bdgt" %EvM added to calc and store pct for Govt dem
  ];
...
% EvM added to store govt demand sa as obs using buget gcons 
dbObsTrans.obs_gcons_Bdgt_y     = db.gcons_Bdgt_y_sa;
dbObsTrans.obs_gdem_Bdgt_y     = db.gdem_Bdgt_y_sa;
