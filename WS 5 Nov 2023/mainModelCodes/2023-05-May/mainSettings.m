...
%% Name of the parameters; setparam1 was ZR set July 15, removed

opts.parameterNames   = "setparam";
opts.parameterLegends = "Orig";

% opts.parameterNames   = ["setparam";  "setparam_fiscImp"];
% opts.parameterLegends = ["Orig.";     "No gdem discr"];

% opts.parameterNames   = ["setparam";  "setparam_alt1";  "setparam_alt2"];
% opts.parameterLegends = ["Orig.";     "No gdem discr";  "Cyc oexp/grev"];

% opts.parameterNames   = ["setparam";  "setparam_alt3"];
% opts.parameterLegends = ["Orig.";     "Std gdem discr"];
...
opts.reportModel.varDecomp.histRange = qq(2006,1) : qq(2022,4);
...
% Range of comparison
opts.reportModel.stdComp.range = qq(2006,1) : qq(2022,4);
...
% Plot range
opts.filterHistory.rangePlot = qq(2015,1) : qq(2023,1);
...
opts.histForecast.range = qq(2015,1) : qq(2022,4);
...
opts.histForecast.variables = [
  ...
  "prem_d_gap"
  "i_tnd"
  % added r_tnd; also in minecofin.model, tune variables and r_tnd AR(1) eq.
  "r_tnd"
  "d4l_cpi_tar"
  "prem"
  "e_dl_z_tnd"
  %"prem_d_tnd"; does not works
  ];
...
% Horizon of forecast
opts.forecast.range = qq(2023,1) : qq(2026,4); % extended forward each Q if new data
...
opts.forecastReport.highlightRange = qq(2015,1) : qq(2022,4);% fr2022=fcastQ3 
