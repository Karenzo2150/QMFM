...
codes.writeMessage("reportModel: loading model ...");
...
if opts.reportModel.steady.run
  ...
  varDescr = codes.selectFromList(varNames, xnamesAll, xdescrAll);
...
if opts.reportModel.IRF.run
  ...
  codes.writeMessage("reportModel: adding impulse responses ...");
  ...
  if ~isequal(optsIRF.shocks, @all)
    ...
    edescr  = codes.selectFromList(enames, enamesAll, edescrAll);
  else
    enames = enamesAll;
    edescr = edescrAll;
  end
  ...
  for i = 1:length(enames)
    ...
    for j = 1:length(xnames)
      ...
      graphTitle = char(xdescr(j) + " [" + xnames(j) + "]");
...
if opts.reportModel.varDecomp.run
  ...
  codes.writeMessage("reportModel: adding variance decompositions ...");
...
if opts.reportModel.stdComp.run
  ...
  compVarDescr = codes.selectFromList(compVarNames, xnamesAll, xdescrAll);
  ...
  compVarDescr = codes.selectFromList(compVarNames, enamesAll, edescrAll);
  rowNames = compVarDescr + " [" + compVarNames + "]";
...
if opts.reportModel.equations.run
  ...
  codes.writeMessage("reportModel: adding equations ...");
...
codes.writeMessage("reportModel: compiling ...");

fileName = fullfile(opts.mainDir, "reports", "modelReport.pdf");
if codes.checkFile(fileName)
  rprt.publish(char(fileName), opts.publishOptions{:});
end

% Close invisible figure windows
codes.closeFigures();

codes.writeMessage("reportModel: done");
