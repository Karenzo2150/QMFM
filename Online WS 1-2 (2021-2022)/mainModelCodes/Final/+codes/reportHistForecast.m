...
% Adjust hifForecast range
tmp = dbrange(dbFcast(1));
...
for n = 1:paramNum
  ...
  data        = db2tseries(rmse(n)) / db2tseries(rmseRW(n));
  rprt.matrix(char(matrixTitle), data(:)',...
    'rownames', cellstr(rowNames), ...
    'colnames', cellstr(colNames), ...
    'format',   '%.3f');
