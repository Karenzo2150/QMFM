...
ind = ~cellfun(@isempty, regexp(ynamesAll, '^tune_'));
...
for n = 1:paramNum
  ...
  for i = 1:length(optsHF.exogvars)
    ...
    if isfield(dbObs, obsName) % Observations in observed data
      tunes.(exogName) = dbObs.(obsName);
    else % Use full-sample historical filtration
      if obsName ~= ""
  ...
  maxLastFcast = dbrange(tunes, 'endDate', 'minRange');
