...
% -------- Calculate prices --------
% NISR CPI index 2014=100 weights (to be revised based on CPI rebasing) 
w_core = 7747/10000;
...
% db.cons(qq(2023, 3) : qq(2025,  1))   = NaN;
% db.inv(qq(2023,  3) : qq(2025,  1))   = NaN;
% db.exp(qq(2023,  3) : qq(2025,  1))   = NaN;
...
for v = varNames(:)'
  db.(v + "_sa") = x13(db.(v), "Missing", true);
end
...
dbAux.dBG_bop_usd = db.dBG_bop_usd; %ak-corr 12/12/23: BOP data, not yet used
...
% -------------------------------------------------------------------------
% ------------- End of main function for master dabase-------------
