...
legend ('Level','Trend', 'interpreter', 'none',...
    'location', 'southoutside', 'orientation', 'horizontal', 'fontsize', 12);
title('Real Exchange Rate (RER), 100*log', 'interpreter', 'none', 'FontSize',12);
fileName = "RER_QQ %";
codes.utils.saveEmf(opts, gcf, fileName);
...
hold on
grid on
plot(plotRange, db.d4l_z, 'linewidth', 1, 'color', 'k');
legend ('ER, Nominal', 'CPI, Foreign','CPI, Core', 'interpreter','none',...
  'location', 'southoutside', 'orientation', 'horizontal', 'fontsize', 12);
highlight(HighRange)
title('Real Exchange Rate (RER), YY%', 'fontsize', 12,...
  'interpreter','none');
fileName = "RER_decomp";
codes.utils.saveEmf(opts, gcf, fileName);
