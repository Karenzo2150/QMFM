...
% This parameter set is recalibrated version of the original set param and contains new calibrated coefficients i.e., policy
% rules reaction inflation, lag in core inflation, UIP in ER. These coefficients recalibrated during Nov 2024 WS 
% -------------------------------
% -------- Steady states --------
...
p.ss_dl_y_agr_tnd         =  100*log(1 + 7.5/100); % IMF-NBR WP has 8.5, check seems high
...
% --- Core inflation --- 
p.b1  = 0.35; % lag,  recalibrated in Nov 2024 WS; reduced so inflation responds faster to its other determinants; before 0.45; IMF WP/20/295 has 0.5, but must be tested
...
p.c2 = 1.2; 0.5; %dl_cpi_dev; %Taylor rule c2 and c3 recalibrated in Nov 2024 WS (higher response to inflation deviation, lower response to outputgap) % dl_cpi_dev, lower end of the reference range, but larger then IMF WP/20/295
p.c3 = 0.2; 0.5; %l_y_gap; recalibrated in Nov 2024; most references have similar value, but NBR-IMF WP/20/295 has lower (0.2)
...
p.e1 = 0.5; 0.2; %recalibrated ER rule e1 in Nov 2024 WS to assume larger role for UIP and expected ER; was 0.5 before (pure target 0, pure UIP 1)
p.e2 = 0.5; 0.2; %recalibrated ER rule e2 in Nov 2024 WS to assume larger role for UIP and expected ER  % Forward looking: 1
p.e3 = 0.9;   % Premium AR
p.e4 = 0.00;  % Target deprc. AR
p.e5 = 0.30;  % 0.30; target deprc. infl. dev.,also BNR has 0.30
p.e6 = 0.85;  % 0.85; target deprc. REER gap, also BNR has 0.85
