...
% Read model and obsevered data
opts.readModel.run                    = 1;

opts.readData.run                     = 1;
opts.reportCompareData.run            = 1;
opts.misc.reportDataRaw.run           = 1; % EvM added to diagnose raw obs data

% Filtration/estimation of unboserved (gaps&trend)
...
opts.misc.reportCompareFilter.run     = 1; % EvM added to diagnose filter results

% Historal/in-sample forecast
...
% Report/plot charts for WP, forcast presentation 
opts.misc.histFcastCharts.run          = 0;
opts.misc.supplemtables.run            = 0;
...
opts.reportModel.varDecomp.histRange = qq(2006, 1) : qq(2023, 4);
...
% Range of comparison
opts.reportModel.stdComp.range = qq(2006, 1) : qq(2023, 4);

%% Historical filter options

% Filtered range
opts.filterHistory.range = qq(2006, 1) : qq(2026, 1); % default 2024Q2 if data; extend filter for shocktuning +2Q
...
opts.histForecast.range = qq(2016, 1) : qq(2024, 4);
...
opts.forecastReport.plotRange = qq(2017, 1) : opts.forecast.range(end); 
...
opts.forecastReport.tableRange = qq(2026, 1) : opts.forecast.range(end); 

% Range for annualized series
opts.forecastReport.tableRange_ann = yy(2021) : yy(2030);
%% Process options
